...
const handleImageUpload = (event) => {
    ...
    // Reset input so user can pick the same file again if they deleted it by mistake
    event.target.value = '';
};
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg dark:bg-gray-800">
                    <div class="p-6">
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div>
                                <InputLabel for="images" value="Project Images" />
                                <input
                                    id="images"
                                    type="file"
                                    multiple
                                ...
                                <div v-if="form.images.length > 0" class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <div v-for="(file, index) in form.images" :key="index" class="relative group aspect-square rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
                                        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                            <button @click.prevent="removeImage(index)" class="text-white bg-red-600 rounded-full p-1 hover:bg-red-700">
